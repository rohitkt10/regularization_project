"""
Task 4 - Comparing the classification performance and interpretability of
shallow and deep CNN models under various regularization settings.
-------------------------------------------------------------------

Subtask 4.3 - Table 1 manifold mixup training results
...
def main():
    ...
    for trial in range(args.start_trial, args.end_trial+1):
        ...
        for i, layer in enumerate(model.layers):
            if 'activation' in layer.name:
                if layer.activation.__name__ != 'linear' and layer.activation.__name__ != 'sigmoid':
                    ks.append(i)
        model = ManifoldMixupModel(model=model, ks=ks, alpha=0.2)
        ...
        # fit the model
        model.fit(
            traindata.shuffle(10000).batch(128),
